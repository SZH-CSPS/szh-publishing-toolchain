#!/usr/bin/env bash
# Construit le rootfs WSL reproductible et l'exporte en .tar (+ sha256).
...
WEASYPRINT_VERSION="${WEASYPRINT_VERSION:-69.0}"
FONTS="${FONTS:-fonts-noto fonts-dejavu}"      # + polices de la revue
IMAGE="szh-publishing:${DISTRO_VERSION}"
OUT="szh-publishing-rootfs-${DISTRO_VERSION}.tar"
...
echo ">> export rootfs (tar à plat pour wsl --import)"
cid="$("${ENGINE}" create "${IMAGE}")"
"${ENGINE}" export "${cid}" -o "${OUT}"

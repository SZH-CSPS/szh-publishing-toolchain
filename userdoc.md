...
- **Articles** – un article par ligne. **Un clic fait tout** : le texte s’ouvre à
  ...
  **Remplacer** au survol l’échange contre un autre fichier HTML, même nom conservé,
  ...
  formulaire qui liste **tous les médias de l’article**. Une carte par **figure**, dans
  l’ordre où elles apparaissent dans le texte  une image seule, ou toutes les images
  d’une même grille rassemblées dans la même carte.

  Une carte est **repliée**. On n’y voit que les **aperçus** – celui de l’image, ou ceux
  des images de la grille, côte à côte – et la zone **« Ajouter une image à côté »**. Sous
  chaque aperçu, son nom de fichier et, s’il y a quelque chose à savoir, des **pastilles** 
  *2 insertions*, *jamais insérée*, *doublon*, *basse résolution*, *image muette*. Ce qui ne
  va pas se voit sans rien ouvrir – c’est la raison d’être de ces pastilles.

  **Un clic sur un aperçu déplie le formulaire de cette image**, sous la rangée entière.
  Un seul à la fois dans une carte  cliquer une autre image de la grille referme le
  précédent, recliquer la même le referme. L’aperçu ouvert est cerné d’un filet de la
  couleur du numéro, et le formulaire redit de quelle image il est.

  Dans ce formulaire 

  - le **nom du fichier**, ses dimensions et son poids, avec deux pictos  **agrandir
    l’aperçu** et **supprimer** (le fichier **et** ses insertions dans le texte, après
    confirmation) 
  - **Remplacer cette image** – une zone de glisser-déposer, ou un clic pour choisir un
  ...
  - sous **Accessibilité**, le **rôle** de l’image pour les lecteurs d’écran et son
    **texte alternatif** 
  ...
  **« Ajouter une image à côté »**, sous les aperçus, est la seule zone visible carte
  repliée  le fichier déposé **s’ajoute**, il n’écrase rien. Les images ne font plus
  qu’**une seule figure**, avec **un numéro et une légende** pour l’ensemble, et le fichier
  entre dans l’article sous son propre nom.

  Elle accepte un glisser-déposer ou un clic pour choisir un fichier, et porte en plus
  **« Une image de l’article… »**, pour ranger côte à côte deux images qui sont déjà là –
  le cas ordinaire après un import Word. Une image déjà insérée dans le texte est alors
  **déplacée** ici, avec son texte alternatif et ses crédits  seule sa légende propre
  disparaît, la figure n’en portant qu’une.

  **Chacune des deux zones demande confirmation, et chacune offre l’issue de l’autre.**
  Déposer sur « Remplacer » propose *Remplacer l’image* / *Plutôt poser la nouvelle à
  côté* / *Annuler*  déposer sur « à côté » propose *Ajouter à côté* / *Remplacer* /
  *Annuler*. C’est le filet du geste qui ne se défait pas  un fichier lâché sur la
  mauvaise des deux zones se rattrape sans avoir à retrouver l’image écrasée.

  On peut mettre **jusqu’à six images** dans une figure  au-delà, il faut scinder en deux
  figures – six vignettes dans la largeur d’une colonne, c’est déjà six petites images.
  Quand c’est impossible – image jamais insérée dans le texte, ou grille déjà pleine – la
  zone « à côté » se grise et dit pourquoi.

  #### Les paramètres du groupe d’images

  Dès qu’il y a deux images dans la carte, un accordéon **« Paramètres du groupe
  d’images »** apparaît sous les aperçus. Il porte ce qui vaut pour la **figure entière**,
  et non pour une image 

  - la **liste des images** qui la composent 
  - la **légende de la figure** – une seule pour toute la grille. L’en-tête de l’accordéon
    la redit, replié  on la lit sans ouvrir, et une figure qui n’en a pas le dit. Les
    images n’ont pas de légende propre  leur texte alternatif et leurs crédits, eux,
    restent à chacune, dans leur propre formulaire 
  - le menu **« Disposition »** 
  ...
  Pour défaire, **deux sorties**, dans le formulaire de chaque image de la grille – ce sont
  des gestes qui portent sur **une** image, pas sur le groupe. Ni l’une ni l’autre ne
  touche à l’image voisine, ni à la figure, qui garde son numéro et sa légende 

  - **Sortir de la grille** – l’image redevient une figure à elle seule, juste après le
    bloc, avec sa légende, ses crédits et son texte alternatif 
  - **Retirer de la figure** – l’image quitte la figure **et** le texte. Le fichier reste
    dans l’article et son aperçu reste dans le formulaire, prêt à être réinséré ailleurs.

  Quand il ne reste qu’une image, la grille se défait toute seule. Et si l’on **supprime**
  carrément le fichier (la corbeille du formulaire de l’image), la figure se remet d’aplomb
  pareil  les images restantes gardent leur numéro, et la disposition suit le nouveau
  compte.

  **Ctrl + S** enregistre toutes les cartes d’un coup et relance la compilation – y compris
  les formulaires repliés, qui n’ont rien perdu. Une image qui n’est **insérée nulle part**
  dans le texte le dit et se verrouille  il n’y a aucun endroit où écrire sa légende.
  Insère-la d’abord (`Ctrl+Alt+F`) – le gestionnaire s’ouvre alors **sur elle**, formulaire
  déplié.
...
- **🚀 Commande** (`Ctrl+Alt+A`) – la gestion de la revue :
  ...
  - **Métadonnées des articles** – un formulaire qui liste **tous les articles** :
    ...
    **L’autocomplétion des auteur·e·s.** Dans la fenêtre d’un·e auteur·e, dès **deux
    ...
    (nom et affiliation) et les **numéros déjà produits sur ce poste** (d’où viennent la
    **fonction** et l’**e-mail**, qu’OJS ne publie pas). Les deux se rafraîchissent seuls,
    au plus **une fois par mois**, en arrière-plan – rien à lancer, et hors ligne ne
    change rien.

    Les **noms de famille** qui commencent par ce que vous tapez viennent en premier,
    puis un trait de séparation, puis les **prénoms** ; la part trouvée est en **gras**.
    Le début de chaque mot compte : « wilde » trouve « Wood de **Wilde** ». On choisit à
    la souris, ou aux flèches puis Entrée. **Nom et prénom sont remplacés ; les autres
    champs ne se remplissent que s’ils sont vides** – une correction que vous venez de
    taper n’est jamais effacée.

    Le champ **ROR** est l’identifiant international de l’institution (celui de la HfH
    est `https://ror.org/00w9q2c06`). Il se remplit tout seul quand la personne est déjà
    publiée. Rempli **avec** l’affiliation en toutes lettres, il part dans l’export OJS
    et rattache l’article à la bonne institution, quelle que soit la manière dont son nom
    est écrit. Seul, sans affiliation, il ne part pas – l’export vous le signale.
...
C’est un filet de sécurité qui permet à l’équipe technique de comprendre une panne sans que
vous ayez à la décrire vous-même.
...
veut aller plus loin : `docs/RAPPORTS-ERREUR.md`.
